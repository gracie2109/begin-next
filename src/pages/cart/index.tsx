            ...
            <Row gutter={[32, 16]} align="stretch">
                <Col xs={24} md={16}>
                    ...
                    <Row justify="space-between">
                        ...
                        <Col>
                            <Typography.Text>Bạn đang có <Typography.Text strong>{cloneList?.length} sản
                                phẩm</Typography.Text> trong giỏ hàng</Typography.Text>
                        </Col>
                    </Row>
                    <Divider/>
                    ...
                    {cloneList && cloneList.length > 0 ? (
                            ...
                            <div className="mb-3">
                                <Row className="my-3" gutter={[8, 8]}>
                                    <Col span={24}>
                                        {money && money > FREE_SHIP_MONEY ? (
                                            <Typography.Text>Bạn đã được <Typography.Text strong>MIỄN PHÍ VẬN
                                                CHUYỂN</Typography.Text></Typography.Text>
                                        ) : (
                                            <Typography.Text>Bạn cần mua thêm <Typography.Text strong>{needMore} để được
                                                MIỄN PHÍ VẬN CHUYỂN</Typography.Text></Typography.Text>
                                        )}
                                    </Col>
                                    ...
                                    <Col span={24}> <Alert message="Bạn có thể click vào sl sp bên trái để xóa sp"
                                                           type="warning" showIcon closable/></Col>
                                </Row>
                                            ...
                                            <List.Item key={item.id} style={{width: "100%"}}>
                                                <List.Item.Meta
                                                    avatar={
                                                        <Badge
                                                            count={<span
                                                                className="cursor-pointer bg-red-600 text-white w-[20px] h-[20px] text-[7px] rounded-full absolute top-0 grid place-items-center"
                                                                onClick={() => {
                                                                    deleteItem(item);
                                                                }}>3</span>} className="pointer-cursor">
                                                            <Avatar src={item?.images?.[0] || item?.images || ""}
                                                ...
                                                <Row align="middle" style={{textAlign: "end"}}>
                                                    <Col span={24}><Typography.Text
                                                        strong>{formatCurrency(item?.price)}</Typography.Text></Col>
                                                    <Col span={24}>
                                                        <InputNumber
                                                            min={1}
                                                            max={99}
                                                            ...
                                                            addonBefore={<Button style={{padding: 0}} type="link"
                                                                                 onClick={() => setValue(Number(value) + 1)}> +</Button>}
                                                            addonAfter={<Button style={{padding: 0}} type="link"
                                                                                disabled={value === 0}
                                                                                onClick={() => setValue(Number(value) - 1)}> -</Button>}
                                                            defaultValue={1}/>
                                                    </Col>
                                ...
                                <Card>
                                    <Row justify="start" gutter={[8, 8]}>
                                        <Col>
                                            {!openVat ? (
                                                <FaRegCheckCircle size={24} onClick={() => setOpenVat(!openVat)}
                                                                  className="cursor-pointer"/>
                                            ) : (
                                                <AiFillCheckCircle size={24} onClick={() => setOpenVat(!openVat)}
                                                                   className="cursor-pointer"/>
                                            )}
                                        </Col>
                                        <Col> <Typography.Text strong onClick={() => setOpenVat(!openVat)}
                                                               className="cursor-pointer"> Xuất hoá đơn cho đơn
                                            hàng </Typography.Text></Col>
                                    </Row>
                                    {openVat && (
                        ...
                        <Row align="middle" justify="center">
                            <Col md={12} xs={12}> <img
                                src="https://theme.hstatic.net/200000592359/1001008166/14/cart_banner_image.jpg?v=147"
                                style={{width: "100%", objectFit: "cover"}}/> </Col>
                ...
                <Col xs={24} md={6}>
                    <Card title={<Typography.Text strong>Thông tin đơn hàng</Typography.Text>}>
                        ...
                        <div className="my-7">
                            <button
                                className="border-none font-bold outline-none text-white uppercase bg-[#ff0000] w-full p-3 rounded-lg">
                    ...
                    <div className="my-7">
                        <Card bodyStyle={{padding: "10px"}} style={{backgroundColor: "#d9edf7"}}>
                            ...
                            <Typography.Paragraph style={{paddingTop: "3px"}}>
                                Hiện chúng tôi chỉ áp dụng thanh toán với đơn hàng có giá trị tối thiểu <Typography.Text
                                strong>{formatCurrency(FREE_SHIP_MONEY)}</Typography.Text> trở lên.
                            </Typography.Paragraph>
                        </Card>
                    </div>
                    <VoucherList/>
