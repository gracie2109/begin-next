...
const Index= () => {
    ...
    const headerActionComp = [
        {
            key: 1,
            comp: <Link href="/admin/sliders/create"> <Button type="dashed" icon={<PlusOutlined />}>Tạo sliders</Button></Link>
        }
    ];
    const tabItems: any[] = [
        {
            key: 1,
            label: `Slider đang hoạt động (${dataActive.length})`,
            children: <FetchData dataSource={undefined} loading={pending} compStatus="active"/>
        },
        {
            key: 2,
            label: `Slider ngừng hoạt động (${dataInActive.length}) `,
